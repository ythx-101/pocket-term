...
export function createStateManager(options) {
  ...
  function ensureRuntime(paneId) {
    ...
    if (!rt) {
      rt = {
        ...
        bubbleSeq: 0,
        /** @type {string|null} M2.8-P4: last broadcast typing-indicator text */
        lastTypingText: null,
      };
  ...
  function pushBubble(paneId, bubble) {
    ...
    // M2.8-P3: Tier B stream cards carry body/tool hierarchy segments so
    // the SPA renders TUI-like structure without re-parsing text.
    if (entry.stream && entry.text) {
      entry.segments = segmentStreamText(entry.text);
    }
  ...
  /**
   * M2.8-P4: in-place typing indicator. The newest spinner line rides a
   * dedicated SSE event — never the bubble buffer / history. Only emitted
   * while the pane is `working` (stale completion notes stay silent);
   * cleared when the spinner vanishes or the status leaves working.
   * @param {string} paneId
   * @param {string} screenText
   */
  function updateTyping(paneId, screenText, ts = Date.now()) {
    const rt = ensureRuntime(paneId);
    const typing =
      rt.lastStatus === 'working' ? latestSpinnerLine(screenText) : null;
    if (typing === (rt.lastTypingText ?? null)) return;
    rt.lastTypingText = typing;
    broadcast('typing', { pane_id: paneId, text: typing, ts });
  }
  ...
  function onStatus(paneId, status, ts = Date.now()) {
    ...
    if (status !== 'working' && rt.lastTypingText != null) {
      rt.lastTypingText = null;
      broadcast('typing', { pane_id: paneId, text: null, ts });
    }
    rt.lastActivity = Math.max(rt.lastActivity, ts);
  ...
  async function ingestPaneOutput(paneId) {
    ...
    // M2.8-P4: latest spinner line → in-place typing indicator (SSE only).
    updateTyping(paneId, text);
    ...
    if (shouldEmitTierB(rt) && rt.openLines.length) {
      ...
      for (const b of folded) {
        list.push({
          ...
          stream: true,
          sealed: false,
          segments: segmentStreamText(b.text),
        });
      }
    }
