...
export function createStateManager(options) {
  ...
  async function resolvePaneTranscript(pane) {
    ...
    if (resolved) {
      // Drop any pending Tier B open lines once Tier A is active.
      rt.openLines = [];
      rt.openTs = null;
    }
    return resolved;
  ...
  async function stop() {
    ...
    // Cancel/await in-flight output loops (and their wait RPCs via AbortSignal).
    const pending = [...outputLoops.values()];
    outputLoops.clear();
    if (pending.length) {
