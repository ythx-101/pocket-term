...
export function createStateManager(options) {
  ...
  /**
   * Rate-limit check for send. Returns null if allowed, else retry-after seconds.
   * @param {string} paneId
   * @param {number} [now]
  ...
  async function sendToPane(paneId, text, mode = 'run') {
    ...
    const rate = checkSendRate(paneId);
    if (!rate.ok) {
      return {
        ok: false,
        status: 429,
    ...
    const method = sendMode === 'text' ? 'pane.send_text' : 'pane.run';
    const params =
      sendMode === 'text'
        ? { pane_id: paneId, text: displayText }
        : { pane_id: paneId, text: displayText };
    ...
    rt.recentUserSends = rt.recentUserSends.filter(
      (s) => ts - (Number(s.ts) || 0) <= USER_BUBBLE_DEDUPE_MS + 5000
    );
  ...
  function sealOpen(paneId) {
    ...
    for (const b of folded) {
      ...
      pushBubble(paneId, { ...b, role: 'agent', stream: true });
    }
  }
        ...
        try {
          ...
          if (rt.transcriptPath) {
            try {
              ...
              try {
                const st = await fs.stat(rt.transcriptPath);
                if (rt.transcriptOffset === 0) rt.transcriptOffset = st.size;
              } catch {
                /* ignore */
              }
            } catch {
