  ...
  async function sendToPane(paneId, text, mode = 'run') {
    ...
    try {
      if (sendMode === 'text') {
        ...
        await client.rpc('pane.run', { pane_id: paneId, text: '' });
      } else {
        // Two-step: type text, pause, bare Enter. A single pane.run bursts
        // text+Enter in one write; Claude Code paste detection drops CJK.
        await client.rpc('pane.send_text', {
          pane_id: paneId,
          text: displayText,
        });
