...
export async function createPushService({ stateDir, config = pushConfigFromEnv(), sender } = {}) {
  ...
  function persist() {
    writeChain = writeChain.then(() => atomicWritePrivate(stateDir, subscriptions));
    return writeChain;
  }
  ...
  async function dispatch(event) {
    ...
    const payload = JSON.stringify({
      title,
      body: String(event?.title || paneId || 'Agent 会话'),
      tag: `pt2-${paneId}-${status}`.slice(0, 180),
      url: `/herd/#/chat/${encodeURIComponent(paneId)}`,
      status,
    });
    if (Buffer.byteLength(payload) > PUSH_PAYLOAD_MAX_BYTES) return { sent: 0, failed: subscriptions.length, pruned: 0 };
