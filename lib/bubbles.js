...
/**
 * Hotkey bar token: `Shift+Tab:mode`, `Ctrl+g:send to bg` (action may
 * contain spaces — Grok's bar does). Joined by `│` or 2+ spaces.
 * Whole-row match only; code echoes that embed one token stay content.
 */
const CHROME_HOTKEY_TOKEN_RE =
  /^(?:shift|ctrl|alt|opt|cmd|esc|tab|enter|space|fn|f\d{1,2})(?:[+-][\w↑↓←→]+)*:\S/i;
...
/**
 * Leading/trailing ornaments on Grok's input-frame edges
 * (`╭╮╯╰─` box corners, ASCII/Unicode dashes, spaces, light verticals).
 * Only peeled at the ends so internal hyphens (`always-approve`) survive.
 */
const GROK_FRAME_EDGE_RE = /^[╭╮╯╰─━═│┃║|—–\-\s]+|[╭╮╯╰─━═│┃║|—–\-\s]+$/g;

/**
 * Grok CLI model/mode status core, after edge ornaments are peeled:
 *   `Grok 4.5 (high) · always-approve`
 *   `Grok 4.5 (high)`
 * Deliberately requires the line to *be* that status (optional mode after
 * `·`), so prose like `Grok 4.5 release notes` or `see always-approve`
 * is kept as body text.
 */
const GROK_STATUS_CORE_RE =
  /^Grok\s+v?\d+(?:\.\d+)*(?:\s*\([A-Za-z0-9_+/.-]+\))?(?:\s*·\s*[A-Za-z0-9_+/.-]+)?$/i;

/**
 * Framed empty prompt inside Grok's input box: `│ ❯ │` / `│ ❯`.
 * Bare `❯` is already chrome via CHROME_BARE_PROMPT_RE.
 */
const GROK_FRAMED_PROMPT_RE = /^[│┃║|\s]*❯[│┃║|\s]*$/;

/**
 * True when a whole line is a hotkey bar (2+ `key:action` tokens joined
 * by `│` or 2+ spaces). Allows spaces inside the action (`send to bg`)
 * and a trailing `│`.
 * @param {string} t trimmed line
 * @returns {boolean}
 */
function isHotkeyBarLine(t) {
  const s = t.replace(/^\s*│\s*/, '').replace(/\s*│\s*$/, '').trim();
  if (!s) return false;
  // Need a join marker; a single `Ctrl+c:cancel` prose fragment is not a bar.
  if (!s.includes('│') && !/\s{2,}/.test(s)) return false;
  const parts = s
    .split(/\s*│\s*|\s{2,}/)
    .map((p) => p.trim())
    .filter(Boolean);
  if (parts.length < 2) return false;
  return parts.every((p) => CHROME_HOTKEY_TOKEN_RE.test(p));
}

/**
 * Grok CLI input-frame chrome. Live TUI draws:
 *   ╭────────────────────────────╮
 *   │ ❯                          │
 *   ╰──── Grok 4.5 (high) · always-approve ─╯
 * Pure box edges are already isDecorativeLine; this catches the status
 * edge (box + model · mode) and the framed empty prompt — patterns the
 * Claude-oriented footer matcher (requires `⎇`) never saw, so they
 * leaked into chat bubbles as the "两条斜杠" under agent replies.
 * @param {string} t trimmed, ANSI-stripped line
 * @returns {boolean}
 */
function isGrokChromeLine(t) {
  if (GROK_FRAMED_PROMPT_RE.test(t)) return true;
  const core = t.replace(GROK_FRAME_EDGE_RE, '').trim();
  if (!core) return false;
  return GROK_STATUS_CORE_RE.test(core);
}
...
export function isChromeLine(line) {
  ...
  if (isGrokChromeLine(t)) return true;
  ...
  if (isHotkeyBarLine(t)) return true;
