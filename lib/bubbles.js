 ...
 * ` · ⎇ ` alone is not enough (prose can mention branches): the row must
 * also carry a version number in its first ` · ` segment or end with a
 * `+N/-N` diffstat.
 */
const CHROME_MODEL_FOOTER_RE = / · ⎇ /;
const CHROME_FOOTER_VERSION_RE = /\d+\.\d+/;
const CHROME_FOOTER_DIFFSTAT_RE = /\+\d+\/-\d+$/;
...
export function isChromeLine(line) {
  ...
  const t = s.trim();
  if (!t) return false;
  if (CHROME_MODEL_FOOTER_RE.test(s)) {
    const firstSegment = t.split(' · ')[0] ?? '';
    if (
      CHROME_FOOTER_VERSION_RE.test(firstSegment) ||
      CHROME_FOOTER_DIFFSTAT_RE.test(t)
    ) {
      return true;
    }
  }
